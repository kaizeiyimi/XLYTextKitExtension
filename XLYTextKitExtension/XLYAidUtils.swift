...
public func combinePainters(handlers: [(attributeName: String, context: CGContext, lineInfo: XLYLineVisualInfo, visualItems: [XLYVisualItem]) -> Void])
    (attributeName: String, context: CGContext, lineInfo: XLYLineVisualInfo, visualItems: [XLYVisualItem]) {
        handlers.forEach {
...
public func fillLineUsedRect(color: UIColor, var corner: CGFloat = 0, cornerFactor: CGFloat? = nil, insets: UIEdgeInsets = UIEdgeInsetsZero)
    (attributeName: String, context: CGContext, lineInfo: XLYLineVisualInfo, visualItems: [XLYVisualItem]) {
        color.setFill()
        let rect = UIEdgeInsetsInsetRect(lineInfo.usedRect, insets)
        if let cornerFactor = cornerFactor {
            corner = rect.height * cornerFactor
        }
...
public func fillCombinedGlyphRects(color: UIColor, var corner: CGFloat = 0, cornerFactor: CGFloat? = nil, insets: UIEdgeInsets = UIEdgeInsetsZero)
    (attributeName: String, context: CGContext, lineInfo: XLYLineVisualInfo, visualItems: [XLYVisualItem]) {
